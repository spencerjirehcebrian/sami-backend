...
class ScheduleService:
    ...
    def __init__(self, db: Session = None):
        ...
        self._cache = {"movies": {}, "cinemas": {}, "cinema_types": {}}

    def _format_utc_time(self, dt: datetime) -> str:
        """Format datetime as ISO string with UTC 'Z' suffix"""
        if dt is None:
            return None
        ...
        try:
            ...
            if date_from:
                try:
                    ...
                    raise ValueError(
                        f"Invalid date_from format: {date_from}. Use ISO 8601 format."
                    )
            ...
            if date_to:
                try:
                    ...
                    raise ValueError(
                        f"Invalid date_to format: {date_to}. Use ISO 8601 format."
                    )
            ...
            if require_date_filter and not (date_from or date_to):
                raise ValueError(
                    "Date filter (date_from or date_to) is required for schedule queries"
                )
            ...
            schedule_data = [
                {
                    ...
                    "time_slot": self._format_utc_time(schedule.time_slot),
                    ...
                    "occupancy_rate": (
                        round((schedule.current_sales / schedule.max_sales) * 100, 2)
                        if schedule.max_sales > 0
                        else 0
                    ),
                    "status": schedule.status,
                    "created_at": self._format_utc_time(schedule.created_at),
                    "updated_at": self._format_utc_time(schedule.updated_at),
                }
    ...
    def schedule_exists(
        self, cinema_id: str, time_slot: datetime, exclude_schedule_id: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            exists_query = self.db.query(Schedule.id).filter(
                Schedule.cinema_id == cinema_id, Schedule.time_slot == time_slot
            )
    ...
    async def check_schedule_conflicts(
        ...
        try:
            ...
            movie_end_time = time_slot + timedelta(
    ...
    async def get_detailed_conflicts(
        ...
        try:
            ...
            conflicts = (
                self.db.query(Schedule, Movie.title, Movie.duration)
                .join(Movie)
                .filter(
                    and_(
                        Schedule.cinema_id == cinema_id,
                        ...
                        func.datetime(
                            Schedule.time_slot,
                            "+"
                            + func.cast(Movie.duration + 30, func.text())
                            + " minutes",
                        )
                        > time_slot,
                    )
            ...
            for schedule, movie_title, movie_duration_val in conflicts.all():
                conflict_end = schedule.time_slot + timedelta(
                    minutes=movie_duration_val + 30
                )
                conflict_results.append(
                    {
                        ...
                        "time_slot": self._format_utc_time(schedule.time_slot),
                        "end_time": self._format_utc_time(conflict_end),
    ...
    async def check_conflicts(
        ...
        try:
            ...
            if cinema_number and not cinema_id:
                ...
                cinema = (
                    self.db.query(Cinema).filter(Cinema.number == cinema_number).first()
                )
                if not cinema:
    ...
    async def create_schedule(
        ...
        try:
            ...
            if has_conflicts:
                ...
                conflict_messages = [
                    conflict["display_time"] for conflict in conflict_details
                ]
                raise ValueError(
                    f"Schedule conflict detected with: {', '.join(conflict_messages)}"
                )
            ...
            if max_sales > cinema.total_seats:
                raise ValueError(
                    f"Max sales ({max_sales}) cannot exceed cinema capacity ({cinema.total_seats})"
                )
            ...
            full_schedule["message"] = (
                f"Schedule created for {movie.title} in Cinema {cinema.number} at {time_slot_parsed.strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    async def update_schedule(
        ...
        try:
            ...
            if time_slot is not None:
                ...
                movie = (
                    self.db.query(Movie).filter(Movie.id == schedule.movie_id).first()
                )
            ...
            if max_sales is not None:
                cinema = (
                    self.db.query(Cinema)
                    .filter(Cinema.id == schedule.cinema_id)
                    .first()
                )
                if max_sales > cinema.total_seats:
                    raise ValueError(
    ...
    async def get_available_time_slots(
        self, cinema_id: str, date: str, movie_duration: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            end_of_day = datetime.combine(
                target_date, datetime.max.time().replace(hour=23, minute=0)
            )  # End at 11 PM
            ...
            for slot in potential_slots:
                ...
                has_conflicts = await self.check_schedule_conflicts(
                    cinema_id=cinema_id, time_slot=slot, movie_duration=movie_duration
                )
                ...
                if not has_conflicts:
                    ...
                    available_slots.append(
                        {
                            "time_slot": self._format_utc_time(slot),
                            "display_time": slot.strftime("%H:%M"),
                            "end_time": self._format_utc_time(movie_end_time),
    ...
    async def check_batch_conflicts(
        ...
        try:
            ...
            for slot_str in time_slots:
                try:
                    parsed_slot = datetime.fromisoformat(
                        slot_str.replace("Z", "+00:00")
                    )
    ...
    async def get_optimized_available_slots_batch(
        ...
        try:
            ...
            existing_schedules = (
                self.db.query(Schedule.time_slot, Movie.duration)
                .join(Movie)
                .filter(
                    and_(
                        Schedule.cinema_id == cinema_id,
            ...
            while current_time <= end_of_day:
                ...
                if not has_conflict:
                    available_slots.append(
                        {
                            "time_slot": self._format_utc_time(current_time),
    ...
    def get_cached_cinema(self, cinema_id: str) -> Optional[Cinema]:
        ...
        cinema = (
            self.db.query(Cinema)
            .options(joinedload(Cinema.cinema_type))
    ...
    def get_cached_cinema_type(self, cinema_type_id: str) -> Optional[CinemaType]:
        ...
        cinema_type = (
            self.db.query(CinemaType).filter(CinemaType.id == cinema_type_id).first()
        )
        if cinema_type:
